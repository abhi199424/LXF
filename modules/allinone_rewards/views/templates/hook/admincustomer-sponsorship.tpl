...
{/if}
		<div>
			<form id='sponsor' method='post'>
				...
				<span style="padding-left: 40px">{l s='You can customize this code' mod='allinone_rewards'} <input type="text" name="sponsorship_custom_code" value="{$sponsorship_custom_code|escape:'htmlall':'UTF-8'}" size="20" maxlength="20" style="width: auto; display: inline-block"/></span>
		...
		{if $sponsor}
				{l s='Sponsor' mod='allinone_rewards'} <a target="_blank" href="?tab=AdminCustomers&id_customer={$sponsor->id|intval}&viewcustomer&token={getAdminToken tab='AdminCustomers'}">{$sponsor->firstname|escape:'htmlall':'UTF-8'} {$sponsor->lastname|escape:'htmlall':'UTF-8'}</a><br/>
				...
				<table cellspacing='0' cellpadding='0' class='tablesorter tablesorter-ice' id='sponsorships_list'>
					...
					{foreach from=$friends item=sponsored}
						...
						<tr {if !$sponsored['active']}class="inactive"{/if}>
							...
							<td><a target="_blank" href="?tab=AdminCustomers&id_customer={$sponsored['id_sponsored']|intval}&viewcustomer&token={getAdminToken tab='AdminCustomers'}">{$sponsored['lastname']|escape:'htmlall':'UTF-8'} {$sponsored['firstname']|escape:'htmlall':'UTF-8'}</a></td>
							...
							<td class="nostrike">{if $sponsored['level_sponsorship']==1}<input type="text" {if $sponsored['deleted']}disabled{/if} style="width: 100%; box-sizing: border-box" name="date_end_{$sponsored['id_sponsorship']|intval}" class="datetimepicker" value="{if $sponsored['date_end']!='0000-00-00 00:00:00'}{$sponsored['date_end']|escape:'htmlall':'UTF-8'}{/if}">{/if}</td>
							<td class="nostrike" style="text-align: center">{if !$sponsored['deleted'] && $sponsored['level_sponsorship']==1}<input class="button" name="submitSponsorshipEndDate" type="submit" value="{l s='Save settings' mod='allinone_rewards'}" onClick="$('#id_sponsorship_to_update').val({$sponsored['id_sponsorship']|intval})">{/if}</td>
						</tr>
					{/foreach}
					</tbody>
				</table>
