				...
				{foreach $product_combinations as $id_product_attribute => $product_combination}
						<tr>
							...
							<td style="text-align: center">
								<input {if $product_combination.gift_allowed}checked{/if} type="checkbox" name="rewards_gift_allowed[]" value="{$id_product_attribute|intval}">
							</td>
							<td style="text-align: center">
								<div class="input-group" style="margin: 0 auto; display: inline-table; vertical-align: middle;">
									<input type="text" name="rewards_gift_price[{$id_product_attribute|intval}]" value="{$product_combination.price|floatval}" style="width: 80px" class="form-control">
									<span class="input-group-addon" style="width: 10px">{$currency->sign|escape:'html':'UTF-8'}</span>
								</div>
