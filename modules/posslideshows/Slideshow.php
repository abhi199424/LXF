...
class Slideshow extends ObjectModel
{
	...
	public static $definition = array(
		'table' => 'posslideshows_slides',
		'primary' => 'id_posslideshow',
		'multilang' => true,
		'fields' => array(
			'active' =>			array('type' => self::TYPE_BOOL, 'validate' => 'isBool', 'required' => true),
			'position' =>		array('type' => self::TYPE_INT, 'validate' => 'isunsignedInt', 'required' => true),
			'cat_selcted' =>		array('type' => self::TYPE_STRING, 'validate' => 'isCleanHtml', 'required' => false),
	...
	public function add($autodate = true, $null_values = false)
	{
		...
		$res &= Db::getInstance()->execute('
			INSERT INTO `'._DB_PREFIX_.'posslideshows` (`id_shop`, `id_posslideshow`)
	...
	public function delete()
	{
		...
		$res &= Db::getInstance()->execute('
			DELETE FROM `'._DB_PREFIX_.'posslideshows`
			WHERE `id_posslideshow` = '.(int)$this->id
		);
	...
	public function reOrderPositions()
	{
		...
		$max = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			WHERE hss.`id_posslideshow` = hs.`id_posslideshow` AND hs.`id_shop` = '.(int)$id_shop
		);
		...
		$rows = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT hss.`position` as position, hss.`id_posslideshow` as id_slide
			FROM `'._DB_PREFIX_.'posslideshows_slides` hss
			LEFT JOIN `'._DB_PREFIX_.'posslideshows` hs ON (hss.`id_posslideshow` = hs.`id_posslideshow`)
			WHERE hs.`id_shop` = '.(int)$id_shop.' AND hss.`position` > '.(int)$this->position
		);

		foreach ($rows as $row)
		{
			$current_slide = new Slideshow($row['id_slide']);
