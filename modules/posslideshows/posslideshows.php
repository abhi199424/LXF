...
class Posslideshows extends Module implements WidgetInterface
{
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->l('Home Category slide');
        $this->description = $this->l('Adds an image slider to your category page.');
        $this->ps_versions_compliancy = array('min' => '1.7', 'max' => _PS_VERSION_);

        $this->templateFile = 'module:posslideshows/views/templates/hook/slider.tpl';
    }
    ...
    public function install()
    {
        ...
        if (parent::install() &&
            ...
            $this->registerHook('displayCatshow') &&
            ...
            foreach ($shops as $shop_id) {
                ...
                $res &= Configuration::updateValue('POSSLIDESHOW_NAV', $this->default_navigation, false, $shop_group_id, $shop_id);
                $res &= Configuration::updateValue('POSSLIDESHOW_PAGI', $this->default_pagination, false, $shop_group_id, $shop_id);
            }
            ...
            if (count($shop_groups_list)) {
                foreach ($shop_groups_list as $shop_group_id) {
                    ...
                    $res &= Configuration::updateValue('POSSLIDESHOW_NAV', $this->default_navigation, false, $shop_group_id);
                    $res &= Configuration::updateValue('POSSLIDESHOW_PAGI', $this->default_pagination, false, $shop_group_id);
                }
            }
            ...
            $res &= Configuration::updateValue('POSSLIDESHOW_NAV', $this->default_navigation);
            $res &= Configuration::updateValue('POSSLIDESHOW_PAGI', $this->default_pagination);
    ...
    protected function installSamples()
    {
        ...
        for ($i = 1; $i <= 3; ++$i) {
            ...
            foreach ($languages as $language) {
				...
				if($i==1){
					$slide->description[$language['id_lang']] = '<div class="desc desc1">
					<div class="info">
					<p class="title1 " data-animation="animated fadeInDown">Thursday Through Saturday</p>
					<p class="title2 " data-animation="animated fadeInLeft">Plant Big Sale</p>
					<p class="title3 " data-animation="animated fadeInRight">75% Off</p>
					<p class="readmore" data-animation="animated zoomIn"><a href="#"><span>Shop Now</span></a></p>
					</div>
					</div>';
				}elseif($i==2){
					$slide->description[$language['id_lang']] = '<div class="desc desc2">
					<div class="info">
					<p class="title1 " data-animation="animated fadeInLeft">The Stone Series</p>
					<p class="title2 " data-animation="animated zoomIn">A Great Addition</p>
					<p class="title3 " data-animation="animated fadeInDown">To Your Home</p>
					<p class="readmore" data-animation="animated zoomIn"><a href="#"><span>Shop Now</span></a></p>
					</div>
					</div>';
				}elseif($i==3){
					$slide->description[$language['id_lang']] = '<div class="desc desc3">
					<div class="info">
					<p class="title1 " data-animation="animated fadeInDown">Workshops @Alula</p>
					<p class="title2 " data-animation="animated zoomIn">Plant Make</p>
					<p class="title3 " data-animation="animated fadeInLeft">People Happy</p>
					<p class="readmore" data-animation="animated zoomIn"><a href="#"><span>Shop Now</span></a></p>
					</div>
					</div>';
				}else{
    ...
    public function uninstall()
    {
        ...
        if (parent::uninstall()) {
            ...
            $res &= Configuration::deleteByName('POSSLIDESHOW_PAGI');
    ...
    protected function createTables()
    {
        ...
        $res &= Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'posslideshows_slides` (
              ...
              `cat_selcted` varchar(100) NOT NULL DEFAULT \'0\',
    ...
    protected function _postProcess()
    {
        ...
        if (Tools::isSubmit('submitSlider')) {
            ...
            foreach ($shops as $shop_id) {
                ...
                $res &= Configuration::updateValue('POSSLIDESHOW_PAGI', (int)Tools::getValue('POSSLIDESHOW_PAGI'), false, $shop_group_id, $shop_id);
            }
            ...
            switch ($shop_context) {
                case Shop::CONTEXT_ALL:
                    ...
                    $res &= Configuration::updateValue('POSSLIDESHOW_PAGI', (int)Tools::getValue('POSSLIDESHOW_PAGI'));
                    if (count($shop_groups_list)) {
                        foreach ($shop_groups_list as $shop_group_id) {
                            ...
                            $res &= Configuration::updateValue('POSSLIDESHOW_PAGI', (int)Tools::getValue('POSSLIDESHOW_PAGI'), false, $shop_group_id);
                        }
                    }
                    break;
            ...
            /* Sets category */
            if(Tools::getValue('cat_selcted')) {
                $selected_catsec = implode(",", Tools::getValue('cat_selcted'));
                $slide->cat_selcted = $selected_catsec;
            }
            else {
                $slide->cat_selcted = '1';
            }
                ...
                if (isset($_FILES['image_'.$language['id_lang']]) &&
                 ...
                 if (isset($_FILES['image_mobile_'.$language['id_lang']]) &&
                     ...
                     if ($error = ImageManager::validateUpload($_FILES['image_mobile_'.$language['id_lang']])) {
                         ...
                         return false;
                     } elseif (!ImageManager::resize($temp_name, __DIR__.'/images/'.$salt.'_'.$_FILES['image_mobile_'.$language['id_lang']]['name'], null, null, $type)) {
    ...
    public function hookdisplayCatshow()
    {
		$id_category = $category['category'];
        $slides = $this->getSlidesCat();
       ...
       if ($slides) {
        ...
        $this->smarty->assign('homeslider', $homeslider);
        return $this->fetch($this->templateFile);
    }

    public function getSlidesCat()
    {
        ...
        $slides = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
            SELECT hss.`cat_selcted`, hs.`id_posslideshow` as id_slide, hss.`position`, hss.`active`, hssl.`title`,
        ...
        foreach ($slides as &$slide) {
            ...
            $slide['image_mobile_url'] = $this->context->link->getMediaLink(_MODULE_DIR_.'posslideshows/images/'.$slide['image_mobile']);
        }
    ...
    public function getWidgetVariables($hookName = null, array $configuration = [])
    {
		...
		foreach($slides as &$slide){
			...
			$slide['description'] = str_replace('/pos_alula/',__PS_BASE_URI__,$slide['description']);
		}
        if (is_array($slides)) {
            foreach ($slides as &$slide) {
                $slide['sizes'] = @getimagesize((dirname(__FILE__) . DIRECTORY_SEPARATOR . 'images' . DIRECTORY_SEPARATOR . $slide['image']));
            ...
            $(function() {
                ...
                $mySlides.sortable({
                    ...
                    update: function() {
                        ...
                        showSuccessMessage("position updated");
                        }
                    });
    ...
    public function renderAddForm()
    {
        ...
        $selected_cat = array();
        if($id_slide) {
            $catSeL = Db::getInstance()->getRow('SELECT `cat_selcted` FROM '._DB_PREFIX_.'posslideshows_slides WHERE id_posslideshow ='.$id_slide);
            $catArray = explode(',',$catSeL['cat_selcted']);
            $selected_cat = $catArray;
        }
        ...
        $fields_form = array(
            'form' => array(
                ...
                'input' => array(
                    array(
                        'type' => 'file_lang',
                        'label' => $this->l('Select a background'),
                        'name' => 'image',
                        'required' => true,
                        'lang' => true,
                    ...
                    array(
                        'type' => 'file_lang',
                        'label' => $this->getTranslator()->trans('Select small icon', array(), 'Admin.Global'),
                        'name' => 'image_mobile',
                        'required' => true,
                        'lang' => true,
    ...
    public function getConfigFieldsValues()
    {
        ...
        return array(
            ...
            'POSSLIDESHOW_PAGI' => Tools::getValue('POSSLIDESHOW_PAGI', Configuration::get('POSSLIDESHOW_PAGI', null, $id_shop_group, $id_shop)),
        );
    }
    ...
    public function getAddFieldsValues()
    {
        ...
        $languages = Language::getLanguages(false);
		 ...
		 foreach ($languages as $lang) {
			...
			$fields['description'][$lang['id_lang']] = str_replace('/pos_alula/',__PS_BASE_URI__,$fields['description'][$lang['id_lang']]);
		}
