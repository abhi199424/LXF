...
include_once(_PS_MODULE_DIR_.'posslideshows/posslideshows.php');

$home_slider = new Posslideshows();
$slides = array();
...
if (Tools::getValue('action') == 'updateSlidesPosition' && Tools::getValue('slides'))
{
	...
	foreach ($slides as $position => $id_slide)
		$res = Db::getInstance()->execute('
			UPDATE `'._DB_PREFIX_.'posslideshows_slides` SET `position` = '.(int)$position.'
			WHERE `id_posslideshow` = '.(int)$id_slide
		);
